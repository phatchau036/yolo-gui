...
- Có Cloud workspace để kết nối Google Drive folder public/shared, tạo chuẩn thư mục chung và dùng cùng cấu trúc dữ liệu giữa Windows, máy khác và Colab.
...
- Bật Cloud mode, nhập Google API key local, kết nối Google Drive folder và tạo mirror theo chuẩn `datasets/models/runs/annotations/configs/exports/logs`.
...
Vào tab `Cài đặt`, bật `Cloud mode`, nhập Google API key và Google Drive folder ID/link, rồi bấm `Connect Google Drive`.
...
Setting Cloud được lưu local trong `logs/cloud/cloud-settings.local.json` và thư mục này bị `.gitignore`. Không commit API key lên GitHub. Có thể dùng biến môi trường `YOLO_GUI_GOOGLE_API_KEY` nếu không muốn lưu key trong file local.

Lưu ý: API key chỉ đọc được Google Drive folder public/shared. Nếu muốn đọc private Drive, upload file, sync hai chiều hoặc mount Drive theo tài khoản cá nhân, bước tiếp theo cần OAuth/service account.
...
Profile được lưu ở `runs/cloud/.../configs/gui-settings/`. Khi mở lại trên máy khác hoặc Colab có cùng Cloud mirror, bấm `Áp dụng` để GUI tự điền lại form. Cloud Manager cũng quét `models`, `datasets`, `runs`, `exports`, `configs`, `annotations` để bấm dùng nhanh model, ảnh hoặc dataset.
...
- `yolo_gui/cloud_manager.py`: lưu Cloud setting local, đọc metadata Google Drive và tạo mirror/manifest theo chuẩn thư mục chung.
...
- `logs/cloud/`: setting Cloud local, gồm API key nếu người dùng chọn lưu từ GUI; thư mục này không được commit.
- `logs/updates/`: log cập nhật phiên bản bằng GUI.
- `logs/system_reports/`: report môi trường.
- `runs/gui_jobs/`: config JSON theo job.
- `runs/cloud/`: mirror local và manifest metadata của Cloud workspace.
