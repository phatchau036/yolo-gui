...
- Có tab `Phiên bản` trên cả Windows và Google Colab để xem changelog, kiểm tra bản mới trên GitHub và bấm cập nhật; Colab tự mở tunnel mới sau update.
...
- Có nút kiểm tra/cập nhật phiên bản GUI từ GitHub nếu repo sạch; trên Colab, GUI giữ phiên cũ tới khi link `trycloudflare.com` mới sẵn sàng.
...
Khi cập nhật trong tab `Phiên bản`, Colab sẽ tự mở server và tunnel mới trước. Đợi panel `Colab tunnel` hiện nút `Mở GUI mới`, sau đó chuyển sang link mới; phiên cũ sẽ tự dừng sau vài giây.
...
- `start_colab.py`: launcher Google Colab, tự chạy server, Cloudflare Tunnel và handoff sang tunnel mới sau update.
