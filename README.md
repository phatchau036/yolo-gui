...
- Các setting nâng cao mở bằng popup để màn chính gọn hơn nhưng vẫn chỉnh được khi cần.
...
- Có Cloud workspace theo luồng 4 bước dễ hiểu: bật Cloud, kiểm tra Cloud API key, thêm Google Drive Auth, rồi GUI tự tạo chuẩn thư mục chung để dùng giữa Windows, máy khác và Colab.
...
Vào tab `Cài đặt`, nhìn khung tiến trình Cloud bên trái và đi theo 4 bước: `Bật Cloud`, `Kiểm tra key`, `Drive Auth`, `Tạo workspace`.

Bật `Cloud mode`, nhập Cloud API key rồi bấm `Kiểm tra Cloud key`. Khi key hợp lệ, GUI mới hiện phần `Google Drive Auth`; dán OAuth access token có quyền Drive rồi bấm `Kết nối và tạo folder`.
