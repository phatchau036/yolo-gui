...
2. Frontend gọi:
   ...
   - `GET /api/dependencies/ultralytics` để hiện trạng thái Ultralytics trên GUI.
   - `GET /api/models` để nạp model preset.
3. Nếu thiếu Ultralytics, người dùng bấm `Cài Ultralytics` trên GUI.
4. Backend chạy `DependencyManager`, spawn `python -m pip install ultralytics>=8.3.0`, ghi log vào `logs/dependency_installs/ultralytics-install.log`.
5. Người dùng nhập hoặc duyệt tới `data.yaml`.
6. Frontend gọi `POST /api/datasets/inspect`.
7. Người dùng chỉnh setting train.
8. Frontend kiểm tra lại Ultralytics rồi gom form thành JSON và gọi `POST /api/train/start`.
...
- `dependency_manager.py`: không được nuốt lỗi pip; log cài đặt phải hiện trên GUI.
