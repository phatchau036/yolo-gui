...
- Google API key và Google Drive token do người dùng nhập chỉ được lưu local trong `logs/cloud/cloud-settings.local.json` hoặc lấy từ env `YOLO_GUI_GOOGLE_API_KEY` / `YOLO_GUI_DRIVE_ACCESS_TOKEN`. Không commit key/token, không ghi key/token vào manifest, không render raw secret trong UI hoặc response.
