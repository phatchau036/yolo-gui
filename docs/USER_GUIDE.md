...
4. Bấm `Tạo và dùng ngay` để GUI tự tạo cấu hình dataset và gán sang các tab khác.
5. Kiểm tra thẻ môi trường. Nếu thiếu PyTorch, CUDA hoặc Ultralytics thì bấm nút cài ngay trên GUI.
6. Vào tab `Huấn luyện`, chọn model, chọn mức `Cân bằng`, để máy chạy `Tự động`, rồi bấm `Bắt đầu huấn luyện`.
7. Vào tab `Tiến trình` để xem trạng thái và nhật ký.
8. Sau khi có model, dùng tab `Dự đoán`, `Đánh giá` hoặc `Đóng gói` theo nhu cầu.
9. Vào tab `Phiên bản` khi muốn xem changelog hoặc cập nhật GUI lên bản mới.
