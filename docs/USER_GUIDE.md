...
3. Nếu muốn dùng chung dữ liệu giữa nhiều máy/Colab, vào tab `Cài đặt`, bật `Cloud mode`, đặt `Tên project`, nhập Google Drive folder và bấm `Connect Google Drive`.
...
4. Nhập `Tên project`, ví dụ `Helmet Detection`. Dùng cùng tên project trên máy khác hoặc Colab để thấy cùng workspace.
5. Bật `Cloud Storage` nếu muốn job xong tự lưu config, log, output và snapshot vào project.
6. Giữ `Tên workspace chuẩn` giống nhau trên các máy nếu muốn cùng cấu trúc.
7. Bấm `Lưu cài đặt Cloud` hoặc `Connect Google Drive`.
