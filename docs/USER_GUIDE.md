...
3. Vào tab `Dữ liệu`, chọn thư mục dataset và nhập danh sách nhãn.
4. Nếu ảnh chưa có nhãn, dùng panel `Vẽ bounding box như LabelImg`: chọn thư mục ảnh, kéo chuột để vẽ box, chọn class rồi bấm `Lưu nhãn`.
5. Bấm `Tạo và dùng ngay` để GUI tự tạo cấu hình dataset và gán sang các tab khác.
6. Kiểm tra thẻ môi trường. Nếu thiếu PyTorch, CUDA hoặc Ultralytics thì bấm nút cài ngay trên GUI.
7. Vào tab `Huấn luyện`, chọn model, chọn mức `Cân bằng`, để máy chạy `Tự động`, rồi bấm `Bắt đầu huấn luyện`.
8. Vào tab `Tiến trình` để xem trạng thái và nhật ký.
9. Sau khi có model, dùng tab `Dự đoán`, `Đánh giá` hoặc `Đóng gói` theo nhu cầu.
10. Vào tab `Phiên bản` khi muốn xem changelog hoặc cập nhật GUI lên bản mới.
