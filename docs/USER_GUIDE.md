...
3. Nếu muốn dùng chung dữ liệu giữa nhiều máy/Colab, vào tab `Cài đặt`, làm theo tiến trình `Bật Cloud` -> `Kiểm tra key` -> `Drive Auth` -> `Tạo workspace`, rồi bấm `Kết nối và tạo folder`.
...
Các mục `Tùy chỉnh nâng cao` trong Train, Đánh giá, Dự đoán và Đóng gói sẽ mở bằng popup. Người mới có thể bỏ qua. Khi cần chỉnh sâu, bấm mục đó, sửa setting rồi bấm lại tiêu đề popup, bấm vùng nền hoặc nhấn `Esc` để đóng.
...
Cách dùng theo đúng thứ tự trên GUI:

1. Nhìn thanh tiến trình bên trái để biết đang ở bước nào.
2. Bật `Cloud mode`.
3. Nhập `Cloud API key`.
4. Bấm `Kiểm tra Cloud key`. Nếu key hợp lệ, GUI mới mở bước Google Drive.
5. Dán `Google Drive Auth` là OAuth access token có quyền Drive. Nếu muốn đặt workspace trong một folder cha có sẵn, nhập thêm `Google Drive parent`; nếu để trống, GUI tạo trong My Drive.
6. Nhập `Tên project`, ví dụ `Helmet Detection`. Dùng cùng tên project trên máy khác hoặc Colab để thấy cùng workspace.
7. Bật `Cloud Storage` nếu muốn job xong tự lưu config, log, output và snapshot vào project.
8. Giữ `Tên workspace chuẩn` giống nhau trên các máy nếu muốn cùng cấu trúc.
